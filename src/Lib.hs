...
loadAndProcess :: String -> String -> IO Bool
loadAndProcess samplesFilePathA samplesFilePathB = do
  ...
  process [inputsA,inputsB]

process :: [[B.ByteString]] -> IO Bool
process bss = do
  ...
  let statsTitle = "2-sample KS statistic;  dist: " ++ show dist ++ " / cAlphaRoot: " ++ formattedCAlphaRoot ++ " / null Hyp. rejected: " ++ show nullHRejected
  let indexedPointsLists = zip [0..] (samples ++ [distCoords])
  let pinfos = map (\ indexedPointsList -> PlotInfo
                     {points = snd indexedPointsList,
                      color = colors !! (fst indexedPointsList),
                      title = (titles ++ [statsTitle]) !! (fst indexedPointsList)}) indexedPointsLists
  doplots pinfos
...
------------------------------------------------------------------------------------------------------------------------
--- plotting
data PlotInfo = PlotInfo { points :: [Coord], color :: Color, title :: String }

doplot :: [Coord] -> IO Bool
doplot cfps = plot Aqua $
  Data2D [Title "CDF", Style Linespoints , Color Blue] [] cfps

doplots :: [PlotInfo] -> IO Bool
doplots pinfos = plot Aqua $ plotInfos2Data2Ds pinfos

-- convert from local record type instances to Data2D instances
plotInfos2Data2Ds :: [PlotInfo] -> [Graph2D Double Double]
plotInfos2Data2Ds = map (\ pinfo -> (Data2D [ Title $ title pinfo, Style Linespoints, Color $ color pinfo] [] (points pinfo)))
